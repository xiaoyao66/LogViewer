...
namespace LogViewer
{
    ...
    public partial class FormMain : Form
    {
        ...
        private void LoadSFTP(string name, Stream sf, bool newTab)
        {
            ...
            if (newTab)
            {
                ...
                tabControl.TabPages.Add(lf.Initialise(name));
                ...
                lf.Load(name, sf, synchronizationContext, cancellationTokenSource.Token);
            }
            else
            {
                ...
                tabControl.SelectedTab.ToolTipText = name;
                lf.Dispose();
                lf.Load(name, sf, synchronizationContext, cancellationTokenSource.Token);
            }
        }
        ...
        private void timerUpdate_Tick(object sender, EventArgs e)
        {
            ...
            LogFile lf = logs[tabControl.SelectedTab?.Tag.ToString()];
            if (lf != null && this.processing == false)
            {
                try
                {
                    if (lf.TryUpdate() && cancelUpdate != null)
                    {
                        this.processing = true;
                ...
                {
                    if (!string.IsNullOrEmpty(lf.FilePath) && !File.Exists(lf.FilePath))
                        return;
                    ...
                    this.cancellationTokenSource = new CancellationTokenSource();
                    menuToolsMultiStringSearch.Enabled = true;
                    lf.Reload(synchronizationContext, cancellationTokenSource.Token);
                }
                finally { }
            }
        }
        ...
        private void openSFTPToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (res == DialogResult.OK)
            {
                try
                {
                    ...
                    LoadSFTP(dlg.Uri.Host + dlg.Uri.PathAndQuery, fs, true);
                }
                catch (Exception ex)
                {
