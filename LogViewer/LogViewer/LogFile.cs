...
namespace LogViewer
{
    ...
    internal class LogFile
    {
        ...
        public void Reload(SynchronizationContext sc, CancellationToken ct)
        {
            Stream fs = FileStream;
        ...
        public void Load(string name, Stream fs, SynchronizationContext _, CancellationToken ct)
        {
            this.Dispose();
            this.FilePath = null;
            this.FileName = name;
        ...
        {
            long res = info.fs.NewData;
            if (res < 0)
                throw new ArgumentOutOfRangeException("文件被截断！");
            return res > 0;
        }
        ...
        public TabPage Initialise(string filePath)
        {
            ...
            colLineNumber.Text = "No.";
            colLineNumber.Width = 85;
            colText.Text = "Data";
        ...
        {
            if (!(e.Model is LogLine ll)) return;
            LogItem li = ll.Item;
            ListViewItem.ListViewSubItemCollection sis = e.Item.SubItems;

            if (sis.Count >= 2)
            {
                e.UseCellFormatEvents = true;
                e.Item.UseItemStyleForSubItems = false;
                ListViewItem.ListViewSubItem lineNo = sis[0];
                lineNo.ForeColor = Color.LightGray;
                lineNo.BackColor = Color.DimGray;

                ListViewItem.ListViewSubItem Item2 = sis[1];

                if (li != null && li.Level > ItemLevel.None)
                {
                    switch (li.Level)
                    {
                        case ItemLevel.Verbose:
                            Item2.ForeColor = Color.DarkGray;
                            break;
                        case ItemLevel.Debug:
                            Item2.ForeColor = Color.DimGray;
                            break;
                        case ItemLevel.Information:
                            Item2.ForeColor = Color.Black;
                            break;
                        case ItemLevel.Warning:
                            Item2.ForeColor = Color.DarkOrange;
                            break;
                        case ItemLevel.Error:
                            Item2.ForeColor = Color.DarkRed;
                            break;
                        case ItemLevel.Fatal:
                ...
                if (ll.SearchMatches.Intersect(this.FilterIds).Any() == true)
                {
                    Item2.BackColor = highlightColour;
                }
                else if (ll.IsFilterLine)
                {
                    Item2.BackColor = filterColour;
                }
                else if (ll.IsContextSearch || ll.IsContextFilter)
                {
                    Item2.BackColor = contextColour;
                }
                else
                {
